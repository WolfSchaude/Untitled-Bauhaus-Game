...
public class BaumenuDetail : MonoBehaviour
{
    ...
    public void showWindow() //Sets detail window active
    {
        if (!detailWindow.activeSelf)
        {
			...
			GameObject.Find("Event_Overview").GetComponent<EventScript>().ToggleEvent();
        }
        else
        {
            detailWindow.SetActive(false);
    ...
    public void updateContent() //Updates detail windows content depending on which button was pressed
    {
        switch (buttonCount)
        {
            case 1:
                ...
                buildingTimeLeftText.text = "Bauzeit übrig: " + GameObject.Find("UI").GetComponent<BaueWerkstatt>().werkBuildTimeInMonths + " Monat(e)";
                ...
                if (GameObject.Find("UI").GetComponent<BaueWerkstatt>().buildInProgress)
                {
